...
VertexOut VS(VertexIn vin)
{
	...
	//vout.PosH = float4(mul(vin.PosL, gWorldViewProj), 1.0f);
...
PixelOut PS(VertexOut pin)
{
	...
	pout.Color += (TexColor * CubeMapColor) * (AmbientTerm + DiffuseTerm + SpecularTerm);
	...
	pout.Color += (TexColor * CubeMapColor) * (AmbientTerm2 + (DiffuseTerm2 + SpecularTerm2) / Attanuation);
	...
	//pout.Color += ((TexColor * CubeMapColor) * (AmbientTerm3 + (DiffuseTerm3 + SpecularTerm3) / Attanuation2)) * SpotCoefficient;
    pout.Color = TexColor;
    ...
    //pout.Depth = gDSB.Sample(SolidState, pin.UV);
