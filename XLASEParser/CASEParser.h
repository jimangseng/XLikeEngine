...
public:
	...
	bool Convert_Optimize(ASEParser::Mesh* pMesh);		// 버텍스(노말), 텍스쳐 값으로 최적화를 해 준다.
	bool ConvertAll(ASEParser::Mesh* pMesh);			// 모두 그냥 변환 (예시용)
...
public:
	...
	ASEParser::Mesh* m_OneMesh;						// 메시 하나의 데이터가 들어갈 구조체
	vector<ASEParser::Mesh*> m_MeshList;				// 메시들
...
private:
	...
	void Create_onetvertex_to_list();	// tvertex 하나를 멤버 리스트에 추가.
};
