...
bool CASEParser::Clear()	// TODO: 리소스 매니저를 통한 메모리 해제
{
	////m_TokenString = { };
	//m_Token = 0;
	//m_parsingmode = 0;
	...
	while (nowtoken = m_lexer->GetToken(m_TokenString), nowtoken != TOKEND_BLOCK_END)
	{
		...
		switch (nowtoken)
		{
			...
			case TOKENR_NODE_NAME:
				...
				// 모드에 따라 넣어야 할 곳이 다르다.
				/// ?? Node_Name과 Node_TM의 Node_Name 중복해서 읽어서 m_nodename에 읽는데, 문제 없을 것 같긴한데..
			...
			case TOKENR_NODE_TM:
				...
				/// 추측. 검증은 조금 이따(?)
				/// 아무것도 읽고싶지 않아도 그냥 비워두면 안된다(?)
				/// GetToken()을 호출해줘야만 다음 토큰으로 넘어간다(?)
			...
			case TOKENR_MESH:
				// 정식주: 아무것도 안해도 어차피 while의 조건식에서 GetToken()을 한다.
				//Parsing_String();
				break;
			...
			case TOKENR_MESH_TVERT:
			{
				...
				{
					if (i != 0)
						Parsing_String(); // *MESH_TVERT 버림. 맨 처음에는 While문 조건식에서 한 번 파싱하므로 제외.
					Parsing_NumberInt(); // TVERT No. 버림.

					Create_onetvertex_to_list();	// T버텍스 하나 만들어서 멤버 벡터에 push_back
					m_OneMesh->m_mesh_tvertex[i]->m_u = Parsing_NumberFloat();	// u 파싱
					m_OneMesh->m_mesh_tvertex[i]->m_v = Parsing_NumberFloat();	// v 파싱
					m_OneMesh->m_mesh_tvertex[i]->m_w = Parsing_NumberFloat();	// w 파싱
				}
			...
			case TOKENR_MESH_TFACE:
			{
				for (int i = 0; i < m_OneMesh->m_mesh_numtfaces; ++i)
				{
					...
					for (int j = 0; j < 3; ++j)
					{
						m_OneMesh->m_meshface[tmpFaceNumber]->m_TFace[j] = Parsing_NumberInt(); // TVertex의 index
			...
			case TOKENR_MESH_NORMALS:
				//Parsing_String();
				/// 재귀함수 탈출의 원인을 찾았다... 이 한 줄로 인해서 호출과 리턴의 짝이 안맞았던 것.. 재귀호출을 못해서..
				//TRACE(m_TokenString);
				break;
